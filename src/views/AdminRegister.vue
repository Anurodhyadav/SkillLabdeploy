<template>
  <div v-if="allowAccess">
    ...
    <div class="form-group">
      <button @click="registerNew" type="submit" class="btn btn-success btn-lg">Add Admin</button>
    ...
    <table class="table table-borderless mx-auto m-2 text-left">
      <tbody>
        <tr v-for="emailItem in emailList" :key="emailItem">
          <td>{{ emailItem }}</td>
          <td>
            <button type="button" class="btn btn-danger m-1" @click="removeAdmin(emailItem)">
              Remove
            </button>
          </td>
        </tr>
      ...
      await db
        ...
        .then((mySnapshot) => {
          mySnapshot.forEach((myDoc) => {
            ...
            db.collection('UserData').doc(this.myEmailId).set({ role: '' }, { merge: true });
          });
        });
        ...
        await db
          ...
          .then((mySnapshot) => {
            mySnapshot.forEach((myDoc) => {
              ...
              db.collection('UserData').doc(this.myUserId).set({ role: 'admin' }, { merge: true });
            });
          });
        if (this.myUserId) {
          this.email = '';
          this.acceptAlert('success', 'Admin Regitered', 'The admin was successfully completed');
        } else {
          this.acceptAlert(
            'danger',
            'User doesnot exits',
