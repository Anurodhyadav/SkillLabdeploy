        ...
        <vs-collapse-item v-for="tests in storedTranscriptions" :key="tests.id">
          <div slot="header" class="header-collapse">
            <div>
              ...
              <span v-else-if="tests.outcome == 'Rejected'" :style="{ color: 'red' }">{{
                tests.date
              }}</span>
              <span v-else-if="tests.outcome == 'Unsettled'" :style="{ color: 'yellow' }">{{
                tests.date
              }}</span>
          ...
          <div>
            <vs-tabs alignment="center" color="white" class="white">
              ...
              >
                ...
                <div>
                  Diff:
                  <span :ref="tests.id + '0' + '---' + test[`test${key + 1}`].audioSelected">{{
                    test[`test${key + 1}`].jsDiff
                  }}</span
                  >%
                  <b-icon
                    ...
                    @click="
                      reDiff(
                        tests.id + '0' + '---' + test[`test${key + 1}`].audioSelected,
                        getTranscriptText(test[`test${key + 1}`]),
                        0,
                      )
                    "
                ...
                <div>
                  Accuracy:
                  <span :ref="tests.id + '1' + '---' + test[`test${key + 1}`].audioSelected">{{
                    test[`test${key + 1}`].accuracy
                  }}</span
                  >%
                  <b-icon
                    @click="
                      reDiff(
                        tests.id + '1' + '---' + test[`test${key + 1}`].audioSelected,
                        getTranscriptText(test[`test${key + 1}`]),
                        1,
                      )
                    "
                ...
                <div>Test Time: {{ getTime(test[`test${key + 1}`].timetaken) }}</div>
              ...
              >
                ...
                <div>
                  Diff:
                  <span :ref="tests.id + '0' + '---' + tests.hardtest.audioSelected">{{
                    tests.hardtest.jsDiff
                  }}</span
                  >%
                  <b-icon
                    icon="arrow-clockwise"
                    ...
                    @click="
                      reDiff(
                        tests.id + '0' + '---' + tests.hardtest.audioSelected,
                        getTranscriptText(tests.hardtest),
                        0,
                      )
                    "
                ...
                <div>
                  Accuracy:
                  <span :ref="tests.id + '1' + '---' + tests.hardtest.audioSelected">{{
                    tests.hardtest.accuracy
                  }}</span
                  >%
                  <b-icon
                    icon="arrow-clockwise"
                    ...
                    @click="
                      reDiff(
                        tests.id + '1' + '---' + tests.hardtest.audioSelected,
                        getTranscriptText(tests.hardtest),
                        1,
                      )
                    "
