<template>
  <div class="Test" v-if="allowAccess">
    <NavBar />
    <VsAlert @closing="startNextStage" :visibility="isBreakPopupVisible" title="Break" type="alert">
    ...
    <div class="testsec">
      <div class="row">
        <div class="col-2">
          ...
          <audio class="audioPlayer" id="match" ref="match" :src="currentAudio" controls></audio>
        </div>
        <div class="col-10">
          <div slot="header">
  ...
  methods: {
    ...
    totalTranscriptText() {
      return this.transcriptions.reduce(
        (accumulator, current) => (current.text === '' ? '' : `${accumulator} ${current.text}`),
        '',
      );
    },
    ...
    async addItem() {
      ...
      if (window.confirm("You're about to submit the test")) {
        ...
        [this.jsDiff, this.accuracy] = await this.diffAlgo(this.currentAudio);
        try {
